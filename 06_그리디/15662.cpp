...
vector<int> turnGears(vector<string>& gears, vector<int>& gear_pointer, int index, int direction) {
    ...
    vector<pair<bool, int>> is_gear_turn;
    is_gear_turn.assign(gears.size(), {false, 0}); // 톱니바퀴가 회전하는지 여부를 저장하는 벡터 초기화
    is_gear_turn[index] = {true, direction}; // 현재 톱니바퀴에 회전 연산 적용

    // 왼쪽으로 톱니바퀴를 회전시킬지 결정
    for (int i = index; i < gears.size() - 1; i++) {
        if (isGearsMatch(gears[i], gears[i + 1], gear_pointer[i], gear_pointer[i + 1]) && is_gear_turn[i].first) {
            if (is_gear_turn[i].second == 1) {
                is_gear_turn[i + 1] = {true, -1}; // 왼쪽 톱니바퀴를 시계 반대 방향으로 회전
            } else {
                is_gear_turn[i + 1] = {true, 1}; // 왼쪽 톱니바퀴를 시계 방향으로 회전
            }
        }
    }
    ...
    for (int i = index; i > 0; i--) {
        if (isGearsMatch(gears[i - 1], gears[i], gear_pointer[i - 1], gear_pointer[i]) && is_gear_turn[i].first) {
            if (is_gear_turn[i].second == 1) {
                is_gear_turn[i - 1] = {true, -1}; // 오른쪽 톱니바퀴를 시계 반대 방향으로 회전
            } else {
                is_gear_turn[i - 1] = {true, 1}; // 오른쪽 톱니바퀴를 시계 방향으로 회전
            }
        }
    }
    ...
    for (int i = 0; i < gears.size(); i++) {
        if (is_gear_turn[i].first) {
            if (is_gear_turn[i].second == 1) {
                cur_gear[i] = (gear_pointer[i] + (TOOTH - 1)) % TOOTH; // 시계 방향 회전
            } else if (is_gear_turn[i].second == -1) {
